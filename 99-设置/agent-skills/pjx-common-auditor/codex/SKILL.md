---
name: pjx-common-auditor
description: Audit project structures and naming for Agent / Skill / Prompt repositories, Obsidian vaults, and documentation-heavy projects. Use for 结构审计, 命名规范审计, 通用项目审计, MOC审计, 知识库审计, pjx-common-auditor, vault audit, structure audit, naming audit, or Obsidian structure audit.
---

# PJX Common Auditor

> Source of truth: `99-设置/agent-skills/pjx-common-auditor/core.md`
> Sync: `python3 99-设置/agent-skills/sync-agent-skills.py --skill pjx-common-auditor`
...
| `---/` 项目目录 | 存在（日期前缀工作区） | 通常不存在 | 课题目录本身即为 `---/` 子目录 |
...
| 项目产出进 `---/` 下日期前缀目录 | `---/2026-04-29-演示方案/` | `---/演示方案/`（无日期前缀） |
...
| 项目目录日期前缀 | `2026-04-29-项目名/` | `项目名/` `04-29-项目名/` |
...
| `---/` 下活跃项目有日期前缀 | `---/2026-04-29-方案A/` | `---/方案A/` |
| 已完成项目从 `---/` 迁移 | 完成 → 归档或保留（有说明） | 2019 年的项目仍留在 `---/` |
...
| `---/课题名/` 不含 wiki 签名 | 按 HXC 项目目录规则（KV-8 工作区卫生：入口索引、日期前缀） |
...
- `99-设置/vault-rules.yaml` — declarative rule base
- `references/naming-rules.md` — detailed software R&D naming rules
