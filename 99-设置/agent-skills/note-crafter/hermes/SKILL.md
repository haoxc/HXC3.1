---
name: note-crafter
description: 创建符合 Vault 规范的知识笔记：命名、定位归档目录、挂 MOC、写 frontmatter。触发词：创建笔记、新建笔记、整理成笔记、生成笔记、create note、new note、draft note。
triggers:
  - "创建笔记"
  - "新建笔记"
  - "整理成笔记"
...
**并行窗口 4**（1 次 execute_code）：合并 MOC 链接追加 + 日记追加。

**总计：4~5 次工具调用**（分类确定时 4 次，需确认时 5 次），预期耗时 25-45 秒。 此目标针对已知主题；若主题陌生需先调研，前置研究阶段另加 2-4 次终端调用（curl/GitHub API/搜索引擎），不计入 4 次窗口配额。研究技巧见 `references/researching-unknown-topics.md`。
