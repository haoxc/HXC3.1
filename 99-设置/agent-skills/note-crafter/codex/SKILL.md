...
Target: 3~4 tool calls per note creation. Load only this skill — hxc-obsidian-vault is now merged in.

**Window 1** (1 call): `skill_view(note-crafter)` — this skill. Skip if already loaded in current session.

**Window 2** (1 precise file/terminal read batch): Use 「Route Cache」 table first (below). If keyword matches, skip vault-index.yaml. Fallback to `99-设置/vault-index.yaml` search_routing only on miss. Merge dedup + MOC read + existing note content read.

**Window 3** (1 write_file): write the note.

**Window 4** (1 edit/write operation): MOC backlink append.

**Total: 3~4 calls (3 if route cache hits), 20-40s.**
...
| general term / concept term | `03-领域/00-概念术语/` |
| discipline term / AI term / management term | Existing terminology directory under the matching `03-领域/{field}/` path |
| topic / research question / create topic | Confirm the topic carrier directory first; do not create a new top-level directory by default |
